...
const Sidebar: FC<SidebarProps> = ({ name,avatar }): ReactElement => {
  ...
  const activeLink = "flex h-[50px] rounded-md cursor-pointer gap-2 p-2 items-center bg-primary-400 text-white";
  const normalLink = "flex h-[50px] rounded-md cursor-pointer gap-2 p-2 items-center bg-white text-[#9E9E9E] hover:bg-neutral-300 hover:text-white";
            ...
            {DataSidebar.map((x, i) => {
              ...
              return (
                ...
                onMouseLeave={() => setHoveredIndex(-1)}>
                   ...
                   style={{ filter: isActive || isHovered ? 'invert(0%)' : 'invert(60%)' }} className={isActive ? activeSvg : normalSvg} src={x.icon}  width={30} height={30} alt={x.title}/>
                      </span>
                      <span>{x.title}</span>
                    </div>
                  </Link>
